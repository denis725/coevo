...
% % index of strategies that use bias (individual learning)
% indind=[find(n(:,istrat,1)==1);find(n(:,istrat,1)==5);...
%     find(n(:,istrat,1)==6);find(n(:,istrat,1)==8)];
indind=find(n(:,istrat,1)==1);
...
%     _____________________________________________________
    ...
    if hyperb~=0
        %if there is hyperbolic discounting
        n(indind,ichoice,t+1)=max(sign(n(indind,ibias,t+1)+...
            hyperb*((n(indind,ichoice,t)==n(indind,isucc,t))*2-1)),0);
        % (each strategy that uses individual learning)
    end
