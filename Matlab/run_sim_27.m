...
gen=8000;              %number of simulated generations
w0=10;              %base fitness
...
param=21;           %number of parameters per individual
...
nstrat=9;           %number of strategies
lambda=1;           %sensitiviy factor for reinforcement learners
kdoubt=2;           %threshold value for use of maj. tal. by IDCs
aversion=1/3         %gives the weight of losses/gains, ie, =1->equal, >1 loss aversion, <1 win affinity
                    %aversion=0 is the same as =1
tallyn=3           %tally number, the number of "screened" individuals

% 'random seed'
% rand('seed',17413)

% 'achtung p mean changed'
...
%   3) time
%   strategies are encoded: 1=indi, 2=majo, 3=success, 4=ITW, 5=OMT, 6=OIL,
%   7=prob. reinf., 8=IDC, 9=omn
% INDICES
...
pfix(1,1:50)=25/100;
pfix(1,51:tmax)=35/100;
pfix(2,1:tmax)=30/100;
...
ind=[...
    ...
    3333;          % MAJority talliers
    3333;          % SUCcess talliers
    3334;          % WEAlth imitators
    0;          % OMTs
    0;          % OILs
...
'achtung2'
% ninitial(1:1000,[ipind ipmaj])=[1/2 1/2];
ninitial(3334:10000,itally)=7;
