...
param=19;           %number of parameters per individual
...
tallyn=7;           %tally/tally#/tally number, the number of "screened" individuals
...
%  11) probability to use individual learning
%  12) probability to use majority tallying
%  13) probability to use success tallying
%  14) probability to use wealth imitation
%  15) probability to use hierarchical strategy 1
%      this strat uses ind. learning if successful, else maj. tallying
%  16) probability to use hierarchical strategy 2
%      this strat uses maj. tallying if successful, else ind. learning
%  17) probability to use reinforcement learning
%  18) whether in this period, the individual has chosen the better of the
%      two options; yes->1, no->0, draw->0.5
%  19) whenever an individual is imitated by ITW, isource gets +1
...
ipind=11;ipmaj=12;ipsuc=13;ipwea=14;iphi1=15;iphi2=16;iprei=17;
ibest=18;isource=19;
...
ind=[...
    ...
    500;          % MAJority talliers
    0;          % SUCcess talliers
    500;          % WEAlth imitators
    0;          % OMTs
    0;          % OILs
...
% 
% ninitial(1:300,iphi1)=ones(300,1);
% ninitial(301:600,iphi2)=ones(300,1);
% ninitial(1:1000,[ipmaj,ipwea])=repmat([.7 .3],nindi,1);
...
%     [meancorr,unik,unikchar,majority_was_right,corrstat,corresp]=...
%         coevo_meancorrect7(pA,pB,n,tmax,nindi,ichoice,...
%         istrat,nstrat,genetics);
...
%     % instead, statistics based on fitness can be taken:
%     [meancorr,unik,unikchar,majority_was_right]=coevo_meancorrect5(...
%         pA,pB,n,tmax,nindi,ichoice,istrat,nstrat,genetics,ifit,w0,b);
    meancorrstat(:,x)=meancorr;
    majority_correct(x)=majority_was_right;
%     correlstat(:,x)=corrstat;
...
%         cellu(2+length(unik),4)=num2cell(sum(stratfreq.*mean(correlstat,2)'));
        ...
        stats=cellu
%         unikchar
%         mean_correct=mean(meancorrstat,2)
% %         geomean_correct=geomean(meancorrstat')'
