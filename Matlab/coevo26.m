...
for t=1:tmax
    ...
    elseif aversion<0   %in case of loss aversion
    ...
    elseif aversion>0   %in case of win affinity
        n(:,ibias,t+1)=q*n(:,ibias,t)-...
...
%         the first choice is random, therefore no costs
    ...
    elseif aversion>0 %if positive info counts more
        for ii=1:n(i,itally,t)
    ...
    elseif aversion<0  %if negative info counts more
        for ii=1:n(i,itally,t)
            scvec=scvec+(n(:,istrat,t)==3).*(2*(...
