...
function [n,pA,pB] = coevo50_bayesian(...
...
% predicting the environments next move
probInc=1-repmat(probs,nindi,1);
probDec=repmat(probs,nindi,1);
% changes in probability due to env. moving
pAbInc=zeros(nindi,length(probs));
pAbDec=pAbInc;
pBbInc=pAbInc;pBbDec=pAbInc;
...
for t=1:tmax
    ...
    % learning based on exponential discounting
    ...
    % ESTABLISH PRIORS FOR BAYESIAN LEARNING ******************************
    
    % UPDATE THE PRIORS
    % FIRST ESTIMATE WHAT THE CURRENT STATE OF ENVIRONMENT IS
    % determine indices of the different the outcomes
    indAs=(n(:,ichoice,t)==1)&(n(:,isucc,t)==1);
    indAf=(n(:,ichoice,t)==1)&(n(:,isucc,t)==0);
    indBs=(n(:,ichoice,t)==0)&(n(:,isucc,t)==1);
    indBf=(n(:,ichoice,t)==0)&(n(:,isucc,t)==0);
    % new priors
    ...
    % NEXT PREDICT THE NEXT STATE OF THE ENVIRONMENT BASED UPON 
    % KNOWLEDGE OF THE UPDATING PROCESS
    % increase
    pAbInc=pAb(:,1:end-1,t+1).*probInc(:,1:end-1);
    pBbInc=pBb(:,1:end-1,t+1).*probInc(:,1:end-1);
    % decrease
    pAbDec=pAb(:,2:end,t+1).*probDec(:,2:end);
    pBbDec=pBb(:,2:end,t+1).*probDec(:,2:end);
    % update priors
    pAb(:,:,t+1)=[zeros(nindi,1) pAbInc]+[pAbDec zeros(nindi,1)];
    pBb(:,:,t+1)=[zeros(nindi,1) pBbInc]+[pBbDec zeros(nindi,1)];
    % NORMALIZE priors
    pAbsum=repmat(sum(pAb(:,:,t+1)'),length(probs),1);
    pBbsum=repmat(sum(pBb(:,:,t+1)'),length(probs),1);
    pAb(:,:,t+1)=pAb(:,:,t+1)./pAbsum';
    pBb(:,:,t+1)=pBb(:,:,t+1)./pBbsum';
    % expected success probabilities of A and B
    expA=sum(pAb(:,:,t+1)'.*repmat(probs,nindi,1)')';
    expB=sum(pBb(:,:,t+1)'.*repmat(probs,nindi,1)')';
    
    % *********************************************************************
    
    
    % 1 BAYESIAN INDIVIDUAL LEARNERS
    % choose option with the higher expected value
    scvec=scvec+(n(:,istrat,t)==1).*sign(expA-expB);
    
        
    % 2 CONFORMISM
    % the threshold for conformists:
    if confSample==3
        tConf=(3-2*x)*x^2;
    elseif confSample==5
        tConf=x^3*(10-15*x+6*x^2);
    elseif confSample==7
        tConf=x^4*(35-84*x+70*x^2-20*x^3);
    elseif confSample==1
        tConf=x;
    end
    
    scvec=scvec+(n(:,istrat,t)==2).*(2*(randIndi<tConf)-1);
    ...
    scvec=scvec+index1.*(2*(randIndi<tConf)-1);
    
    % if unsuccessful last period, use individual learning
    scvec=scvec+((n(:,istrat,t)==3)&(n(:,isucc,t)==0)).*sign(expA-expB);
    ...
    scvec=scvec+((n(:,istrat,t)==4)&(n(:,isucc,t)==1)).*sign(expA-expB);
    
    
    % 5 IN DOUBT, CONFORM
    
    % if there is doubt, use conformism
    index1=((n(:,istrat,t)==5)&(abs(expA-expB)<kdoubt));
    scvec=scvec+index1.*(2*(randIndi<tConf)-1);
    
    % if certain, use individual learning
    scvec=scvec+(n(:,istrat,t)==5).*(abs(expA-expB)>=kdoubt).*sign(expA-expB);
