function [n,pA,pB] = coevo_pbsl_01(...
...
%     end
    ...
    for ii=1:nindi
        if n(ii,istrat,t)==14
            if sum(AABB(1:2,ii))>=2
                scvec(ii)=1;
            else
                scvec(ii)=-1;
            end
            if sum(AABB(:,ii)==[1 1 1 0]')==4
                scvec(ii)=-1;
            elseif sum(AABB(:,i)==[0 2 1 0]')==4
                scvec(ii)=-1;
            elseif sum(AABB(:,ii)==[1 0 1 1]')==4
                scvec(ii)=1;
...
%     _____________________________________________________
    ...
    for iii=1:nindi
        if n(iii,istrat,t)==5
            for ii=1:n(iii,itally,t)                          %screen a # of indis according to tally #
                scvec(i)=scvec(i)+...                       
                    (n(iii,isucc,t)==0).*...                  %if not successful this round
                    (2*(n(randtmat(iii,ii),ichoice,t)==1)-1); %favor corresponding choice
            end
        end
    end
    ...
    for iii=1:nindi
        if n(iii,istrat,t)==6
            for ii=1:n(iii,itally,t)                          %screen a # of indis according to tally #
                scvec(iii)=scvec(iii)+...                       
                    (n(iii,isucc,t)==1).*...                  %if successful this round
                    (2*(n(randtmat(iii,ii),ichoice,t)==1)-1); %favor corresponding choice
            end
        end
    end
    ...
    for iii=1:nindi
        if n(iii,istrat,t)==8
            for ii=1:n(iii,itally,t)                          %screen a # of indis according to tally #
                scvec(iii)=scvec(iii)+...                       
                    (abs(n(iii,ibias,t))<=kdoubt).*...             %if undecided this round
                    (2*(n(randtmat(iii,ii),ichoice,t)==1)-1); %favor corresponding choice
            end
        end
    end
